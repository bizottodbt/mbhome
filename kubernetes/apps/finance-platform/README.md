...
GitRepository:  flux-system/finance-platform
Kustomization:  flux-system/finance-platform
Deploy key:     flux-system/finance-platform-deploy-key
```
...
1. The `denis/finance-platform` Forgejo repo exists.
2. `make forgejo-gitops-deploy-key FORGEJO_GITOPS_APP=finance-platform` has created the deploy key Secret.
