...
PROXMOX_HOME_ASSISTANT_TF_DIR := infrastructure/terraform/proxmox-home-assistant-vm
PROXMOX_HOME_ASSISTANT_TF_VARS := $(PROXMOX_TF_SHARED_VARS) $(if $(wildcard $(PROXMOX_HOME_ASSISTANT_TF_DIR)/terraform.tfvars),-var-file=terraform.tfvars,) $(if $(wildcard $(PROXMOX_HOME_ASSISTANT_TF_DIR)/terraform.local.tfvars),-var-file=terraform.local.tfvars,)
...
VAULT_APP_NAMESPACE ?= immichframe
VAULT_APP_SERVICE_ACCOUNT ?= vault-sync
VAULT_APP_ROLE ?= app-$(VAULT_APP_NAMESPACE)
VAULT_APP_POLICY ?= app-$(VAULT_APP_NAMESPACE)
LLM_NAMESPACE ?= llm
...
openstack-provisioning-ntp: ## Configure the openstack VM as NTP source for IPA on provisioning VLAN
	cd $(ANSIBLE_DIR) && ansible-playbook $(ANSIBLE_INVENTORY) playbooks/openstack-provisioning-ntp.yaml
...
proxmox-home-assistant-vm-init: ## Initialize Terraform for the Home Assistant OS VM
	cd $(PROXMOX_HOME_ASSISTANT_TF_DIR) && terraform init

proxmox-home-assistant-vm-plan: ## Plan the Home Assistant OS VM
	cd $(PROXMOX_HOME_ASSISTANT_TF_DIR) && terraform plan $(PROXMOX_HOME_ASSISTANT_TF_VARS)

proxmox-home-assistant-vm-apply: ## Create/update the Home Assistant OS VM
	cd $(PROXMOX_HOME_ASSISTANT_TF_DIR) && terraform apply $(PROXMOX_HOME_ASSISTANT_TF_VARS)

proxmox-home-assistant-vm-destroy: ## Destroy the Home Assistant OS VM
	cd $(PROXMOX_HOME_ASSISTANT_TF_DIR) && terraform destroy $(PROXMOX_HOME_ASSISTANT_TF_VARS)
...
cloudflared-token-secret: ## Create/update the Cloudflare Tunnel token secret from CLOUDFLARED_TUNNEL_TOKEN
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	@test -n "$$CLOUDFLARED_TUNNEL_TOKEN" || (echo "Set CLOUDFLARED_TUNNEL_TOKEN"; exit 1)
	$(KUBECTL_ADMIN) create namespace "$(CLOUDFLARED_NAMESPACE)" --dry-run=client -o yaml | $(KUBECTL_ADMIN) apply -f -
	$(KUBECTL_ADMIN) -n "$(CLOUDFLARED_NAMESPACE)" create secret generic "$(CLOUDFLARED_TOKEN_SECRET)" --from-literal=token="$$CLOUDFLARED_TUNNEL_TOKEN" --dry-run=client -o yaml | $(KUBECTL_ADMIN) apply -f -

cloudflared-required-secrets-check: ## Confirm Cloudflare Tunnel token exists before Flux reconciles cloudflared
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	@$(KUBECTL_ADMIN) -n "$(CLOUDFLARED_NAMESPACE)" get secret "$(CLOUDFLARED_TOKEN_SECRET)" >/dev/null || (echo "Missing $(CLOUDFLARED_NAMESPACE)/$(CLOUDFLARED_TOKEN_SECRET). Run: export CLOUDFLARED_TUNNEL_TOKEN='...' && make cloudflared-token-secret"; exit 1)

cloudflared-status: ## Show Cloudflare Tunnel pods and recent logs
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	$(KUBECTL_ADMIN) -n "$(CLOUDFLARED_NAMESPACE)" get deploy,pods -l app.kubernetes.io/name=cloudflared -o wide
	$(KUBECTL_ADMIN) -n "$(CLOUDFLARED_NAMESPACE)" get secret "$(CLOUDFLARED_TOKEN_SECRET)" -o custom-columns='NAME:.metadata.name,TYPE:.type' || true
	$(KUBECTL_ADMIN) -n "$(CLOUDFLARED_NAMESPACE)" logs deploy/cloudflared --tail=50 --prefix=true || true
...
vault-oidc-bootstrap: ## Interactively configure Vault OIDC auth against Dex
	...
	$(KUBECTL_ADMIN) -n vault exec "$(VAULT_POD)" -- sh -ec 'set -eu; accessor=$$(vault read -field=accessor sys/auth/oidc); for mapping in "$(VAULT_ADMIN_GROUP):$(VAULT_ADMIN_POLICY)" "$(VAULT_USER_GROUP):$(VAULT_USER_POLICY)" "$(VAULT_READER_GROUP):$(VAULT_READER_POLICY)"; do group="$${mapping%%:*}"; policy="$${mapping#*:}"; vault write identity/group name="$$group" type="external" policies="$$policy" >/dev/null; group_id=$$(vault read -field=id identity/group/name/"$$group"); alias_id=""; for id in $$(vault list -format=json identity/group-alias/id 2>/dev/null | tr -d "[]\",\r" || true); do alias_name=$$(vault read -field=name identity/group-alias/id/"$$id" 2>/dev/null || true); alias_accessor=$$(vault read -field=mount_accessor identity/group-alias/id/"$$id" 2>/dev/null || true); if [ "$$alias_name" = "$$group" ] && [ "$$alias_accessor" = "$$accessor" ]; then alias_id="$$id"; break; fi; done; if [ -n "$$alias_id" ]; then vault write identity/group-alias/id/"$$alias_id" name="$$group" mount_accessor="$$accessor" canonical_id="$$group_id" >/dev/null; else vault write identity/group-alias name="$$group" mount_accessor="$$accessor" canonical_id="$$group_id" >/dev/null; fi; done'
	$(KUBECTL_ADMIN) -n vault exec "$(VAULT_POD)" -- sh -ec 'rm -f "$$HOME/.vault-token"'
...
vault-secrets-operator-bootstrap: ## Interactively configure Vault Kubernetes auth for Vault Secrets Operator
	...
	@printf '%s\n' 'path "$(VAULT_KV_MOUNT)/metadata/platform" {' '  capabilities = ["list"]' '}' '' 'path "$(VAULT_KV_MOUNT)/metadata/platform/*" {' '  capabilities = ["read", "list"]' '}' '' 'path "$(VAULT_KV_MOUNT)/data/platform/*" {' '  capabilities = ["read"]' '}' | $(KUBECTL_ADMIN) -n vault exec -i "$(VAULT_POD)" -- vault policy write "$(VAULT_SECRETS_OPERATOR_POLICY)" -
...
vault-app-namespace-bootstrap: ## Grant one app namespace access only to mbhome/apps/<namespace>
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	@test -n "$(VAULT_APP_NAMESPACE)" || (echo "Set VAULT_APP_NAMESPACE before running this target"; exit 1)
	@echo "Enter a Vault token with enough privilege to configure app namespace policy and Kubernetes auth role."
	$(KUBECTL_ADMIN) -n vault exec -it "$(VAULT_POD)" -- vault login
	-$(KUBECTL_ADMIN) -n vault exec "$(VAULT_POD)" -- vault auth enable -path="$(VAULT_KUBERNETES_AUTH_MOUNT)" kubernetes
	$(KUBECTL_ADMIN) -n vault exec "$(VAULT_POD)" -- vault write auth/"$(VAULT_KUBERNETES_AUTH_MOUNT)"/config kubernetes_host="https://kubernetes.default.svc:443"
	@printf '%s\n' 'path "$(VAULT_KV_MOUNT)/metadata/apps/$(VAULT_APP_NAMESPACE)" {' '  capabilities = ["read", "list"]' '}' '' 'path "$(VAULT_KV_MOUNT)/metadata/apps/$(VAULT_APP_NAMESPACE)/*" {' '  capabilities = ["read", "list"]' '}' '' 'path "$(VAULT_KV_MOUNT)/data/apps/$(VAULT_APP_NAMESPACE)" {' '  capabilities = ["read"]' '}' '' 'path "$(VAULT_KV_MOUNT)/data/apps/$(VAULT_APP_NAMESPACE)/*" {' '  capabilities = ["read"]' '}' | $(KUBECTL_ADMIN) -n vault exec -i "$(VAULT_POD)" -- vault policy write "$(VAULT_APP_POLICY)" -
	$(KUBECTL_ADMIN) -n vault exec "$(VAULT_POD)" -- vault write auth/"$(VAULT_KUBERNETES_AUTH_MOUNT)"/role/"$(VAULT_APP_ROLE)" bound_service_account_names="$(VAULT_APP_SERVICE_ACCOUNT)" bound_service_account_namespaces="$(VAULT_APP_NAMESPACE)" audience="$(VAULT_SECRETS_OPERATOR_AUDIENCE)" token_policies="$(VAULT_APP_POLICY)" token_ttl="1h"
	$(KUBECTL_ADMIN) -n vault exec "$(VAULT_POD)" -- sh -ec 'rm -f "$$HOME/.vault-token"'
...
immich-album-sync-status: ## Show Immich Album Sync CronJob, jobs, pods, and Vault sync
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	$(KUBECTL_ADMIN) -n immich-album-sync get cronjob,jobs,pods
	$(KUBECTL_ADMIN) -n immich-album-sync get vaultauth,vaultstaticsecret,secret
	@$(KUBECTL_ADMIN) -n immich-album-sync get secret immich-album-sync -o go-template='{{range $$k,$$v := .data}}{{$$k}}{{"\n"}}{{end}}' 2>/dev/null || true

immichframe-status: ## Show ImmichFrame app, Vault sync, route, and endpoint status
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	$(KUBECTL_ADMIN) -n immichframe get deploy,pods,svc,httproute
	$(KUBECTL_ADMIN) -n immichframe get vaultstaticsecret immichframe || true
	$(KUBECTL_ADMIN) -n immichframe get secret immichframe -o custom-columns='NAME:.metadata.name,TYPE:.type' || true
	@$(KUBECTL_ADMIN) -n immichframe get secret immichframe -o go-template='{{range $$k,$$v := .data}}{{$$k}}{{"\n"}}{{end}}' 2>/dev/null || true
	$(KUBECTL_ADMIN) -n immichframe logs deploy/immichframe -c render-config --tail=50 || true
	@curl -Ik https://immichframe.apps.mbhome.biz || true

llm-status: ## Show local LLM app, Vault sync, route, PVCs, and Ollama models
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	$(KUBECTL_ADMIN) -n "$(LLM_NAMESPACE)" get deploy,pods,svc,pvc,httproute
	$(KUBECTL_ADMIN) -n "$(LLM_NAMESPACE)" get vaultauth,vaultstaticsecret || true
...
llm-models: ## List models installed in the Ollama PVC
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	$(KUBECTL_ADMIN) -n "$(LLM_NAMESPACE)" exec deploy/ollama -- ollama list

llm-model-pull: ## Pull a model into Ollama (usage: make llm-model-pull LLM_MODEL=qwen2.5-coder:3b)
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
...
forgejo-postgres-secret: ## Write the Forgejo Postgres application owner secret to Vault
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
...
forgejo-config-secret: ## Generate/write Forgejo internal app secrets to Vault
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	@set -eu; \
...
forgejo-admin-secret: ## Write the initial Forgejo admin secret to Vault
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
...
forgejo-oauth-secret: ## Write the shared Dex/Forgejo OAuth client secret to Vault
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
...
forgejo-required-secrets-check: ## Confirm Forgejo secrets have been synced by Vault Secrets Operator
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
...
forgejo-runner-status: ## Show Forgejo runner deployment, Vault sync, policy, PVC, and recent logs
	@test -f "$(KUBECONFIG_FILE)" || (echo "Run make talos-kubeconfig first"; exit 1)
	$(KUBECTL_ADMIN) -n "$(FORGEJO_RUNNER_NAMESPACE)" get deploy,pods,pvc
	$(KUBECTL_ADMIN) -n "$(FORGEJO_RUNNER_NAMESPACE)" get vaultauth,vaultstaticsecret || true
